...
function background(property, compactable, validator) {
  ...
  var libs = [image, position, size, repeat, attachment, origin, clip, color];
...
function borderRadius(property, compactable) {
  ...
  target.libs = fourValues(target, compactable);

  var remainder = _wrapDefault(property.name, property, compactable);
  remainder.value = values.slice(splitAt + 1);
  remainder.libs = fourValues(remainder, compactable);

  for (var j = 0; j < 4; j++) {
    target.libs[j].multiplex = true;
    target.libs[j].value = target.libs[j].value.concat([['/']]).concat(remainder.libs[j].value);
  }
...
function fourValues(property, compactable) {
  var componentNames = compactable[property.name].libs;
  var libs = [];
...
function multiplex(splitWith) {
  return function (property, compactable, validator) {
    ...
    // split over commas, and into libs
    for (i = 0, l = splitsAt.length; i <= l; i++) {
      ...
      splitlibs.push(splitWith(_property, compactable, validator));
    }

    var libs = splitlibs[0];

    // group component values from each split
    for (i = 0, l = libs.length; i < l; i++) {
      libs[i].multiplex = true;

      for (j = 1, m = splitlibs.length; j < m; j++) {
        libs[i].value.push([MULTIPLEX_SEPARATOR]);
        Array.prototype.push.apply(libs[i].value, splitlibs[j][i].value);
      }
    }
...
function widthStyleColor(property, compactable, validator) {
  ...
  var libs = [
    _wrapDefault(descriptor.libs[0], property, compactable),
    _wrapDefault(descriptor.libs[1], property, compactable),
    _wrapDefault(descriptor.libs[2], property, compactable)
  ];
  var color, style, width;

  for (var i = 0; i < 3; i++) {
    var component = libs[i];
