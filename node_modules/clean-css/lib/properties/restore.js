...
function background(property, compactable, lastInMultiplex) {
  ...
  for (var i = libs.length - 1; i >= 0; i--) {
    ...
    if (component.name == 'background-clip') {
      ...
      var positionComponent = libs[i - 1];
...
function borderRadius(property, compactable) {
  if (property.multiplex) {
    ...
    for (var i = 0; i < 4; i++) {
      ...
      horizontal.libs.push(horizontalComponent);

      var verticalComponent = shallowClone(property);
      verticalComponent.value = [component.value[2]];
      vertical.libs.push(verticalComponent);
    }
...
function fourValues(property) {
  var libs = property.libs;
  var value1 = libs[0].value[0];
  var value2 = libs[1].value[0];
  var value3 = libs[2].value[0];
  var value4 = libs[3].value[0];
...
function multiplex(restoreWith) {
  return function (property, compactable) {
    ...
    for (i = 0, l = property.libs[0].value.length; i < l; i++) {
      if (property.libs[0].value[i][0] == MULTIPLEX_SEPARATOR)
        multiplexSize++;
    }
    ...
    for (i = 0; i <= multiplexSize; i++) {
      ...
      for (var j = 0, m = property.libs.length; j < m; j++) {
        var componentToClone = property.libs[j];
        var _component = shallowClone(componentToClone);
        _property.libs.push(_component);
