...
function wouldBreakCompatibility(property, validator) {
  for (var i = 0; i < property.libs.length; i++) {
    var component = property.libs[i];
...
function isComponentOf(shorthand, longhand) {
  return compactable[shorthand.name].libs.indexOf(longhand.name) > -1;
}
...
function overrideShorthand(property, by) {
  ...
  for (var i = 0, l = property.libs.length; i < l; i++) {
    override(property.libs[i], by.libs[i], property.multiplex);
  }
}
...
function overridingFunction(shorthand, validator) {
  for (var i = 0, l = shorthand.libs.length; i < l; i++) {
    if (anyValue(validator.isValidFunction, shorthand.libs[i]))
      return true;
  }
...
function wouldResultInLongerValue(left, right) {
  ...
  if (left.multiplex) {
    component = multiplexClone.libs.filter(nameMatchFilter(simpleClone))[0];
    overrideIntoMultiplex(component, simpleClone);
  } else {
    component = simpleClone.libs.filter(nameMatchFilter(multiplexClone))[0];
  ...
  for (i = properties.length - 1; i >= 0; i--) {
    ...
    for (j = i - 1; j >= 0; j--) {
      ...
      if (!left.shorthand && right.shorthand && isComponentOf(right, left)) {
        ...
        if (!sameVendorPrefixesIn([left], right.libs))
          continue;

        if (!anyValue(validator.isValidFunction, left) && overridingFunction(right, validator))
          continue;

        component = right.libs.filter(nameMatchFilter(left))[0];
        ...
        component = left.libs.filter(nameMatchFilter(right))[0];
        ...
        // merge if all libs can be merged
        ...
        for (k = left.libs.length - 1; k >= 0; k--) {
          var leftComponent = left.libs[k];
          var rightComponent = right.libs[k];
        ...
        // border is a shorthand but any of its libs is a shorthand too
